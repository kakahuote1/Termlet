...
这些示例用于本地试用和二次开发。示例默认引用 `../../dist/index.mjs`，所以请先构建一次：
...
http://127.0.0.1:4177/examples/renderer-kit/
http://127.0.0.1:4177/examples/windows-style/
http://127.0.0.1:4177/examples/blog-easter-egg/
```

不要直接用 `file://` 打开 HTML。浏览器加载 ESM 模块通常需要 HTTP，否则会被 CORS 或模块加载规则拦截。
...
| `renderer-kit/` | 使用 Renderer Kit 改造输入、输出和动效。 |

...
- virtual filesystem and CTF content
...
Move current fake Linux files, logs, flags, aliases, and CTF clues into a site plugin:

```js
export function kakahuotePreset(terminal) {
  terminal.fs.addFile('/var/log/auth.log', '...');
  terminal.fs.addFile('/root/flag.txt', '...', { owner: 'root', perm: '-rw-------' });
